...
var viewCartItemKeys = ['view cart', 'show cart', 'display cart', 'view', 'display', 'cart'];
...
//TODO: THIS IS HACK// Create crat in appropiate place.
  ...
  if(OPTIONS.includes(custRequest)) {
    console.log("Execution of executeOption");
      choiceHandler.executeOption(customerId, custRequest, function(customerReply) {
  ...
  console.log("SearchResault is " + util.inspect(searchResult, {showHidden: false, depth: null}));

  var addItemChoice = choiceHandler.createChoiceOnSearchSuggestion(customerId, searchResult, function(addItemChoice) {
    console.log("addItemChoice is " + util.inspect(addItemChoice, {showHidden: false, depth: null}));

...
var deleteCart = function(cartId) {
  getCart(cartId, function(cart) {
    console.log("cart" + util.inspect(cart, {showHidden: false, depth: null}));
    cart.status= "DELETED";
...
var createCart  = function (customerId) {
  ...
  getLatestCustomerCart(customerId, function(existingCart) {
    console.log("Existing cart is " + util.inspect(existingCart, {showHidden: false, depth: null}));
...
function addItemsToCart(cartId, itemQuantityToAddInCart) {
  getCart(cartId, function(cart) {
      ...
      for(item in itemQuantityToAddInCart) {
        if(item in existingQuantityMap) {
          console.log("existingQuantityMap1");
            existingQuantityMap[item] = existingQuantityMap[item] + itemQuantityToAddInCart[item];
          }
          else {
    ...
    var updatedCart = new Cart(cartId, cart.customerId, cart.cartCreationTime, existingQuantityMap);
  ...
  getCart(cartId, function(cart) {
    ...
    if (typeof(existingQuantityMap) != "undefined") {
      for(item in itemQuantityToAddInCart) {
        if(item in existingQuantityMap) {
          console.log("existingQuantityMap1");
            existingQuantityMap[item] = existingQuantityMap[item] - itemQuantityToRemoveFromCart[item];
          }
          else {
  ...
  docClient.query(params, function(err, data) {
    console.log("Query result getLatestCustomerCart" + customerId);
      if (err) {
          ...
          console.log("Query succeeded." + util.inspect(data, {showHidden: false, depth: null}));
          if(data.Items.length == 0) {
            callback(null);
          }
          data.Items.forEach(function(item) {
              console.log(" getLatestCustomerCart :  " +  util.inspect(item, {showHidden: false, depth: null}));
              callback(item);
          });
      }
  });
...
var getCart = function(cartId, callback) {
 ...
 var displayCart = function(currentCart) {
  ...
  if(currentCart.cartItems.length == 0 ) {
    return "<br>Cart is currently empty.</br>";
  }
  for(var asin in currentCart.cartItems) {

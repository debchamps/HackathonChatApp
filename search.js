...
var InventoryMap = {};
var COMPLETE_MATCH_THRESHOLD = .8;
var PARTIAL_MATCH_THRESHOLD = .5;
...
var searchInventory = function(query) {
  ...
  for (var index = 0; index < InventoryListing.length; index++) {
    ...
    if(matchRation > COMPLETE_MATCH_THRESHOLD) {
      ...
      console.log("commonItems: " + commonItems + " , matchRation " + matchRation + " Item : "+ itemDesc);
...
function loadInventory() {
 ...
 obj.from.path('./inventory.csv').to.array(function (data) {
  for (var index = 0; index < data.length; index++) {
    var inventory  = new Inventory(data[index][0], data[index][1], data[index][2], 1);
    InventoryListing.push(inventory);
    InventoryMap[inventory.asin] = inventory;
  }
  console.log("Loaded inventory completely of size " + InventoryListing.length);
});
